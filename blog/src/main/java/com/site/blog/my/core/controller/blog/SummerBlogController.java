...
@Controller
public class SummerBlogController {


    //public static String theme = "default";
    //public static String theme = "yummy-jekyll";
//    summer  amaze
    public static String theme = "summer";
    @Resource
    ...
    public String search(HttpServletRequest request, @PathVariable("keyword") String keyword, @PathVariable("page") Integer page) {
        ...
        if (blogPageResult.getTotalCount() == 0){
            blogPageResult =null;
        }
        request.setAttribute("blogPageResult", blogPageResult);
        ...
        request.setAttribute("newBlogs", blogService.getBlogListForIndexPage(1));
        request.setAttribute("hotBlogs", blogService.getBlogListForIndexPage(0));
        //gan 获取所以分类，并且标记当前分类是哪个，当没有匹配时就是-1，-1在thymeleaf模板中不会被解析
        List<BlogCategory> categoryCount = categoryService.getAllCategories();
        int thisCategory = -1;
        for (BlogCategory bc : categoryCount){
            if ( bc.getCategoryName().equals(keyword)){
                thisCategory = bc.getCategoryId();
            }
        }
        request.setAttribute("thisCategory", thisCategory);
    ...
    public String tag(HttpServletRequest request, @PathVariable("tagName") String tagName, @PathVariable("page") Integer page) {
        ...
        request.setAttribute("pageUrl", "tag/" + tagName);
